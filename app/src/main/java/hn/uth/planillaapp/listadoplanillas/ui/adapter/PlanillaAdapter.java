...
public class PlanillaAdapter extends RecyclerView.Adapter<PlanillaAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull PlanillaAdapter.ViewHolder holder, int position) {
        Planilla planillaMostrar = dataset.get(position);
        holder.binding.tvConcepto.setText(planillaMostrar.getConcepto());
        holder.binding.tvFecha.setText(planillaMostrar.getFecha());
        holder.binding.tvTotals.setText(planillaMostrar.getTotals());
        holder.setOnClickListener(planillaMostrar, manejadorEventoClick);
    }
